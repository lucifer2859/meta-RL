...
class AC_Network():
    def __init__(self, a_size, scope, trainer):
        with tf.variable_scope(scope):
            ...
            lstm_cell = tf.nn.rnn_cell.LSTMCell(48, state_is_tuple=True) # tf.nn.rnn_cell.LSTMCell(num_units, state_is_tuple=True)
            c_init = np.zeros((1, lstm_cell.state_size.c), np.float32) # (1, 48)
            h_init = np.zeros((1, lstm_cell.state_size.h), np.float32) # (1, 48)
...
class Worker():
    ...
    def train(self, rollout, sess, gamma, bootstrap_value):
        ...
        advantages = discount(advantages, gamma) # phi=1: MC Methods
        ...
        with sess.as_default(), sess.graph.as_default():                 
            while not coord.should_stop():
                ...
                if episode_count % 50 == 0 and episode_count != 0:
                    ...
                    mean_cumulative_regret = np.max(self.env.bandit) * mean_length - mean_reward
...
with tf.device("/cpu:0"):  
    ...
    for i in range(num_workers):
        workers.append(Worker(dependent_bandit(bandit_difficulty), i, a_size, trainer, model_path, global_episodes))
